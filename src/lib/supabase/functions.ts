...
export interface YouTubeTranscriptFailure {
  ok: false;
  error: string;
  /** Manual fallback link (e.g. https://notegpt.io/...) shown when our
   *  automated pipeline can't extract the transcript. */
  fallbackUrl?: string;
  videoId?: string;
  playabilityStatus?: string;
  debug?: { tried?: string[]; lastError?: string; lastStatus?: string };
}

export type YouTubeTranscriptResponse = YouTubeTranscriptSuccess | YouTubeTranscriptFailure;

export const youtubeTranscriptApi = {
  fetch(url: string, lang?: string) {
    return invokeFunction<YouTubeTranscriptResponse>('youtube-transcript', { url, lang });
  },
};
