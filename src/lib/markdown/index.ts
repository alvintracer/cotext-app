...
export function appendMessage(existingContent: string, message: string, attachments?: string[]): string {
...
export function parseBlocks(content: string): Array<{ timestamp: string; content: string; isPushed: boolean }> {
  const blocks: Array<{ timestamp: string; content: string; isPushed: boolean }> = [];
  const lines = content.split('\n');
  let currentBlock: { timestamp: string; content: string; isPushed: boolean } | null = null;
