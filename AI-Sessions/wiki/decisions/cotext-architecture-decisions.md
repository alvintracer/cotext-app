...
### D-010 Addendum ? Track B Metered Beta (2026-06-18)

- Managed mode now has a real workspace-scoped credit ledger, not just metadata.
- Metering rule for the beta cut:
  - charge only on successful managed extraction
  - estimate cost from request text volume on the server
  - write balance + lifetime usage + transaction row atomically through `apply_managed_credit_usage`
- Default beta grant is currently `100` credits per workspace, with current rows normalized directly in Supabase.
- If charging fails after extraction, the graph result is still returned and the billing error is surfaced as metadata. This preserves user output during beta while keeping the ledger best-effort.
