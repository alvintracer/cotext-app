...
> 파일을 올릴 때 "텍스트만 추출해 메모에 삽입"(바이너리는 안 올림). 지금 이미지 OCR처럼 문서도.

### 30.1 추출기 (`src/lib/extract/index.ts`)

```text
extractText(file) 타입별 분기 (heavy lib는 동적 import = 코드 스플릿):
- 이미지 : 기존 OCR(tesseract)           구조 낮음
- DOCX   : mammoth → HTML → Turndown → md  구조 높음(제목/리스트/굵게/표 일부)
- PDF    : pdf.js getTextContent(최대100p, 진행률)  구조 중간
- hwpx   : JSZip 해제 → OWPML <hp:t> 추출  구조 낮음(문단). 완전판은 서버 변환 Later
- txt/md/csv/json/log : file.text()
라이브러리: pdfjs-dist, mammoth, jszip (turndown·tesseract 보유). vite-env.d.ts(?url) 추가.
```

### 30.2 UX (MorphingComposer)

```text
- 문서 파일 프리뷰에 "텍스트 추출/Extract" 버튼(이미지 OCR과 동일 UI·진행률 링)
- 추출 → 컴포저에 "## 파일명" 헤더와 함께 삽입(다중 파일 구조·출처 유지)
- 추출 성공 시 파일을 업로드 목록에서 자동 제거 → "텍스트만 붙고 파일은 안 올라감"
- 후속: 추출로 구조 깨진 결과를 §31 "Fix with Agent"로 곧장 정리 가능
```

---

## 31. Draft → Fix with Agent (정리/가공·원본 대체) (구현됨)

> draft 블록을 에이전트에 던져 레포 맥락에 맞게 정리·재구조화하고, 결과를 로컬에 추가/원본 대체.

### 31.1 흐름

```text
1. draft 3-dot → "Fix with Agent" → 블록 텍스트가 우측 패널로 (seed)
2. 패널이 재구조화 프롬프트 자동 입력+전송 (레포 grounding):
   "맥락·지식 근거로 정리·재구조화, 의미 보존, 마크다운 구조 복원, 오탈자 교정,
    없는 사실 금지, 깔끔한 md만 출력" (한/영). 결과 스트리밍.
3. 결과에 "추가(+)" / "원본 대체(↻)" 버튼
4. source:<model> 태그 → 클릭으로 source:me 전환 (기존 기능)
```

### 31.2 핵심 설계 — 로컬 콘텐츠 적용 (draft 충돌 회피)

```text
- draft 블록은 GitHub이 아니라 RoomView 로컬 콘텐츠(local_drafts)에만 존재
  → fix 결과는 push가 아니라 RoomView 로컬 콘텐츠에 적용(handleContentChange).
- "추가": appendMessage(content, 결과, source) → 새 draft 블록
- "원본 대체": 원본 timestamp 블록 제거 + 새 블록 추가 (둘 다 로컬, push 없음)
- 배선: RoomView onFixWithAgent(text, ts) → WorkspaceDetailPage(seed + fixOriginTs)
  → AgentPanel(seed 자동전송, onApply/canReplace) → RoomView apply prop(nonce 효과)
- 파일: AgentPanel.tsx, RoomView.tsx, WorkspaceDetailPage.tsx, styles/agent.css
```

### 31.3 후속 (Later)

```text
- 멀티턴 fix(여러 번 대체), 특정 블록 in-place 인라인 편집
- 이미지/문서 추출 직후 "구조 깨졌으니 Fix" 원클릭 제안(§30 연계)
```

## 32. Neural Link — 노드·클러스터·엣지 그래프 (계획, 결정 D-009)

> repo 안 블록을 노드로 묶고 클러스터/엣지로 연결해, 사람과 에이전트가 "이 내용이 무엇과 연관되는지"를 한눈에 보고 찾게 한다.
> 포지셔닝: 인간이 그리는 그래프가 아니라 **에이전트가 읽는 컨텍스트 그래프**.

### 32.1 핵심 결정 (요약)

```text
- 노드 = 블록(파일 아님). 기존 timestamp+source 를 ID로 재사용. 라벨 보유.
- 클러스터 > 개별 freelink. 엣지: 클러스터 경유(암묵) + 노드↔노드 직접(타입 옵션).
- 하이브리드 저장: repo 정본 + Supabase 파생 인덱스(재생성 가능, D-001/D-004 유지).
  - in-repo(무료): markdown 주석 / .cotext/neural.json 이 정본.
  - cross-repo(유료): Supabase 정본 + .cotext/neural-link.json export 스냅샷.
- 단일 쓰기 경로: nodify()/attachCluster()/linkEdge() 순수 lib → UI 버튼·MCP 도구 공용.
  사람은 키 없이 동작, 에이전트 자동화는 같은 경로에 얹는 선택(의존 아님).
```

### 32.2 데이터 모델

```text
Node    : { id, repo, room, blockTs, label, clusters[], createdBy(source) }
Cluster : { id, name, color?, desc? }
Edge    : { from(nodeId), to(nodeId), type?, viaCluster? }

markdown 정본 포맷 (provenance 주석 확장, 비파괴):
  <!-- node: id=n_a1b2 label="가격 정책 v2" clusters=[pricing, gtm] -->
  <!-- edge: n_a1b2 -> n_c3d4 type=supersedes -->
→ parseBlocks 가 이미 주석을 읽으므로 확장만. push 시 .cotext/neural.json 자동 갱신.
```

### 32.3 무료 / 유료 경계

```text
무료(GitHub-native): 노드화·라벨·클러스터, 단일 레포 백링크 패널, 레포 내 클러스터 검색,
                     단일 레포 그래프 뷰(전환 미끼)
유료(Supabase):      크로스 레포 클러스터 검색·그래프, 실시간 인덱스,
                     에이전트 그래프 컨텍스트 API(MCP) — 차별점
```

### 32.4 단계 (사람 우선, 사람·에이전트 대칭 경로)

```text
P0 스키마 + 단일 쓰기 lib(src/lib/neural: 타입 + nodify/attachCluster/linkEdge,
   markdown 주석 read/write, .cotext/neural.json 직렬화) + Supabase 인덱스 테이블 설계
P1 사람 수동 캡처: 블록 → 노드화 → 라벨 → 클러스터 검색 팝업 (키 불필요, 가치 완결)
P2 백링크/관련 패널: 읽는 중 같은 클러스터·엣지 텍스트 노출
...
```text
- 크로스 레포 엣지 정본: Supabase 정본 + export 스냅샷 (채택, P3에서 확정)
- 네이밍: "Neural Link"는 Neuralink 상표 혼동 위험 → Synapse/Mesh/Neural Graph 후보 보류
```
